...
export default function MainLayout() {
  ...
  /* ── Public site is always light, regardless of the user's saved theme
     preference ──
     MainLayout renders only public, non-auth-guarded routes. We never
     read or write ThemeContext's `mode` or localStorage here — the
     guarded app keeps its own independent preference completely
     untouched. We just force the DOM's resolved appearance to light for
     as long as this layout is mounted (including guarding against a
     'system' mode OS-theme change flipping `dark` back on mid-visit via
     ThemeProvider's own listener), and restore whatever was actually
     applied to the DOM the moment the user navigates into the guarded
     app. */
  useEffect(() => {
    ...
    // Re-assert light if something else (e.g. ThemeProvider's system-theme
    // listener) mutates the class/attribute while we're mounted.
  ...
  // On /login, treat nav as solid — but NOT when menu is open
  // (overlay covers everything; nav must be transparent so no double-bg strip).
  const effectiveSolid = menuOpen ? false : (isLoginPage ? true : navSolid);
