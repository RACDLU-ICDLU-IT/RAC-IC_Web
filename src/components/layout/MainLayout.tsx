...
export default function MainLayout() {
  ...
  // Non-hero pages always get this; hero pages only get it after scrolling.
  // When the menu is open the overlay covers everything — navbar must always
  // be transparent so the logo/close icon sit on the branded overlay, not a
  // double-coloured strip. On non-hero pages the navbar is otherwise always solid.
  ...
  useEffect(() => {
    ...
    if (menuOpen) {
      ...
      // opacity + translateY are GPU-composited on mobile → zero repaint, zero jank.
      // No clipPath (causes software rasterisation on low-end Android).
      ...
      // Stagger nav links — tight delay since overlay appears instantly
      if (linksRef.current) {
        gsap.fromTo(
  ...
  /* ─────────────────────────────────────────────────────────────────────
     COLOUR LOGIC
  ───────────────────────────────────────────────────────────────────── */
  // Desktop link colour: solid state → accent on light primary, white on dark primary.
  // Transparent hero state → always white (hero image underneath is dark/branded).
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <nav
        className={`fixed w-full z-50 transition-all duration-500 flex items-center justify-between px-5 md:px-10 lg:px-16 ${
          navSolid
            ? `h-[72px] bg-[var(--color-primary)]/95 backdrop-blur-xl shadow-[0_1px_0_rgba(0,0,0,0.06)] border-b ${onPrimaryBorder}`
      ...
      >
          ...
          {settings.logoUrl && !logoError ? (
            <div
              ...
              style={{
                ...
                width:  navSolid ? '120px' : '132px',
                height: navSolid ? '38px'  : '40px',
                flexShrink: 0,
              }}
            ...
            <span className={`font-heading font-extrabold tracking-tight transition-all duration-300 text-2xl ${
              menuOpen ? 'text-white' : navSolid ? (isLight ? 'text-[var(--color-accent)]' : 'text-white') : 'text-white'
            }`}>
              {tenant.shortName}
            </span>
          )}
        ...
        <div className="flex items-center gap-3 relative z-50">
          ...
          <button
            ...
            className={`lg:hidden relative w-10 h-10 flex items-center justify-center rounded-full transition-all duration-200 hover:bg-white/10 ${hamburgerClass}`}
      ...
      <div
        ...
        className="fixed inset-0 z-40 hidden flex-col"
          backgroundColor: overlayBg, willChange: 'opacity, transform',
        }}
      >
        ...
        <div className="relative z-10 flex flex-col flex-1 overflow-y-auto px-7 pt-28 pb-8">
          ...
          >
            ...
            {/* Thin divider before secondary group */}
            <div style={{
              height: '1px',
...
/* ─────────────────────────────────────────────────────────────────────────
   MOBILE NAV ITEM
   Silicon Valley style: clean label, hover line-through effect replaced
   with a crisp accent dot + slide. No numbers, no arrows crowding.
───────────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────────
   CUSTOM HAMBURGER ICON — three lines, refined
───────────────────────────────────────────────────────────────────────── */
function MenuIcon() {
  return (
