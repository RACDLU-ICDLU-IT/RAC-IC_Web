...
export default function MainLayout() {
  ...
  // Pages where the hero image sits behind the transparent navbar
  ...
  const hamburgerClass = menuOpen
    ? 'text-white'
    : (!isHeroPage || (scrolled && isLight)) ? 'text-gray-800' : 'text-white';

  const logoColor = menuOpen
    ? '#ffffff'
    : (!isHeroPage || (scrolled && isLight)) ? 'var(--color-accent)' : '#ffffff';
        ...
        >
          {settings.logoUrl && !logoError ? (
            ...
            <span className={`font-heading font-extrabold tracking-tight transition-all duration-300 text-2xl ${
              menuOpen ? 'text-white' : scrolled ? (isLight ? 'text-[var(--color-accent)]' : 'text-white') : 'text-white'
            }`}>
              {tenant.shortName}
            </span>
          )}
