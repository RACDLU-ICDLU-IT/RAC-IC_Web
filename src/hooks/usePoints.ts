...
export function usePoints() {
  ...
  const saveDonationPointConfig = useCallback(async (config: Omit<DonationPointConfig, 'id'>): Promise<void> => {
    ...
    setLoading(true);
    try {
      // FIX: id used to be built as `${tenant.id}-dpc-${config.fund_account}`
      // (e.g. "racdlu-dpc-administrative") — a text slug written into
      // donation_point_config.id, which is a uuid column. Postgres rejected
      // every save with 22P02 "invalid input syntax for type uuid". Same
      // category of bug as the level_config fix below: generate a real
      // UUID instead. This only matters on first insert per
      // (tenant_id, fund_account) — the upsert's onConflict target means
      // existing rows keep their original id on every later save.
      const { error } = await supabase.from('donation_point_config').upsert(
        { id: crypto.randomUUID(), tenant_id: tenant.id, ...config, updated_by: user?.id, updated_at: new Date().toISOString() },
        { onConflict: 'tenant_id,fund_account' }
      );
      if (error) handleError(error);
      // record_id must also be a uuid; there's no single natural id here
      // (config is fund-scoped, not row-scoped from the caller's view), so
      // use a fresh uuid to represent this audit event — same approach as
      // saveLevelConfig's logAudit call above.
      await logAudit(tenant.id, 'donation_point_config', crypto.randomUUID(), 'update', user?.id, config);
