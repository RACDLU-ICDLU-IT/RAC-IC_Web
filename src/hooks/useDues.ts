 ...
 * `membership_conditions_text` was added alongside the pre-form
 * Membership Conditions & Dues gate (Join.tsx's 'conditions' step /
 * AdminApplications.tsx's conditions editor). Both are optional on the
 * TYPE (not required) specifically so existing callers that only ever
 * touch club_prefix/default_bkash_number — e.g. AdminDuesSettings.tsx —
 * keep type-checking with zero changes on their end. Only
 * updateDuesSettings' merge behavior (below) actually needs to know
 * these fields exist.
 */
export interface DuesSettings {
  ...
  membership_conditions_text?: string | null;
}
  ...
  const fetchDuesSettings = useCallback(async (): Promise<DuesSettings> => {
    try {
      const { data, error } = await supabase.from('dues_settings')
        .select('club_prefix, default_bkash_number, application_fee, membership_conditions_text').eq('tenant_id', tenant.id).maybeSingle();
      if (error) { console.warn('[dues] settings fetch failed', error); return { club_prefix: '', default_bkash_number: null, application_fee: 0, membership_conditions_text: '' }; }
      return {
        ...
        membership_conditions_text: data?.membership_conditions_text || '',
      };
    } catch {
      return { club_prefix: '', default_bkash_number: null, application_fee: 0, membership_conditions_text: '' };
    }
  }, [tenant.id]);
