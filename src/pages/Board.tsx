...
/* Reordered so board-priority members (filled first) land in a
   contiguous, near-center cluster: p6-p3-p5-p8-p14 are all mutually
   adjacent hexes around the grid's visual center. */
const SLOTS_18 = [
  { cls: 'b-p6',  logo: false },
  { cls: 'b-p3',  logo: false },
  { cls: 'b-p5',  logo: false },
...
const HEX_STYLES = `
  ...
  /* SVG-stroke border — perfectly uniform thickness on all 6 sides */
  .b-hex-svg-border {
  ...
  /* Board ring handled inline via SVG polygon stroke — no separate CSS needed */
`;
...
/* ═══════════════════════════════════════════════════════════════
   HEX GRID
   • Board members always sorted first → land in top/center slots
   • SVG polygon stroke for perfectly uniform border on all 6 sides
═══════════════════════════════════════════════════════════════ */
function HexGrid({
  ...
  // Board members first (preserving their relative sort_order),
  // non-board after — guarantees board cluster in top/center slots
  const sorted = [
    ...members.filter(m => m.is_board_member),
    ...members.filter(m => !m.is_board_member),
  ];

  let memberCursor = 0;
  const assignments = slots.map(slot => {
    if (slot.logo) return { member: null, isLogo: true, origIdx: -1 };
    const member   = sorted[memberCursor] ?? null;
    // origIdx for activeIdx tracking must reference sorted array position
    const origIdx  = memberCursor;
    memberCursor++;
    return { member, isLogo: false, origIdx };
  });
  ...
  // SVG polygon points are defined inline in the render below

  return (
    <div className="b-grid-wrap" style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
      <div className={containerCls}>
        {slots.map((slot, i) => {
          const { member, isLogo, origIdx } = assignments[i];
          ...
          const isActive = !isEmpty && activeIdx === origIdx;

          const isDimmed = boardMode
            ? !isBoard
            : (activeIdx !== null && !isActive);

          // SVG viewBox must match the hex aspect ratio exactly: width=100, height=86.6025
          // Polygon points at 25%/75% of each axis — matches the clip-path exactly
          // stroke-width=4 with half bleeding out; clip is handled by overflow:hidden on parent
          const showBorder  = isActive || (boardMode && isBoard);
          const strokeColor = 'rgba(212,19,103,1)';
          const strokeW     = 4;
          ...
          return (
            <div
              ...
              style={{
                ...
                opacity: isDimmed ? (isEmpty ? 0.07 : 0.2) : 1,
                transform: isActive ? 'scale(1.1)' : 'scale(1)',
                transition: 'opacity 0.28s ease, transform 0.25s ease',
                cursor: isEmpty ? 'default' : 'pointer',
                overflow: 'hidden',
              }}
              onClick={() => !isEmpty && setActiveIdx(isActive ? null : origIdx)}
            >
              {/* ── Photo / placeholder — clipped to hex shape ── */}
              <div
              ...
              >
                {isEmpty ? (
                  ...
                  <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center',
                    justifyContent:'center', backgroundColor:'rgba(212,19,103,0.1)',
              ...
              {/* ── SVG border — only when active or board-highlighted ── */}
              {!isEmpty && showBorder && (
                <svg
                  className="b-hex-svg-border"
