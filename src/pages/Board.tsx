...
function HexGrid({
  ...
  // SVG polygon points for a flat-topped hex in a 100×86.6 viewBox
  // Uniform 3px stroke; half (1.5px) bleeds outside — clip via overflow:hidden on parent
        ...
        {slots.map((slot, i) => {
          ...
          const strokeColor = isEmpty
            ...
            : isActive
              ...
              : (boardMode && isBoard)
                ? 'rgba(212,19,103,0.9)'
                : 'rgba(212,19,103,0.22)'; // subtle always-on ring
          const strokeWidth = (isActive || (boardMode && isBoard)) ? STROKE_W + 1 : STROKE_W;

          const photoFilter = (isActive || (boardMode && isBoard))
            ? 'grayscale(0) brightness(1.05)'
            : 'grayscale(1) brightness(0.72)';

          // inner content inset (px) — slightly inside the stroke
          const inset = 2;
          ...
          return (
            ...
            >
              ...
              {/* ── SVG uniform border — rendered on top of photo ── */}
              {!isEmpty && (
                <svg
