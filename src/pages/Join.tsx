  ...
  return (
    ...
    >
      ...
      <p className="text-xs mt-1.5" style={{ color: inGraceWindow ? '#b45309' : '#3b82f6' }}>
        {inGraceWindow
          ? "Your 48-hour window has passed. We've given you an additional 24 hours — complete payment within it, or this application will be automatically rejected."
          : 'Complete payment within 48 hours of applying, or this application will be automatically rejected.'}
      </p>
    </div>
  );
}
...
export default function Join() {
  ...
  const onSubmit = async (data: JoinFormData) => {
    // photoUrl isn't a react-hook-form field (it's the CloudinaryUpload
    // widget's own state), so joinSchema's zod resolver never validates
    // it — enforced explicitly here instead. Checks both the freshly
    // uploaded photoUrl and, when editing, whatever photo already exists
    // on the application, so re-saving an edit doesn't demand a re-
    // upload if a photo is already on file.
                    ...
                    <button
                      ...
                      style={
                        bkashCopied
                          ...
                          : { backgroundColor: '#fbcfe8', color: '#be185d' }
                      }
                    >
                      {bkashCopied ? (
                        <>
