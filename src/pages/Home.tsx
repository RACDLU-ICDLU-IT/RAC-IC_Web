...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const syncHeight = () => {
      ...
      if (rightH <= 0 || leftW <= 0) return;
      const h1 = leftEl.querySelector('h1') as HTMLElement;
      if (!h1) return;

      // Step 1: binary-search font-size to match RIGHT column height
      let lo = 24, hi = 300;
      h1.style.fontSize = '';
      for (let i = 0; i < 25; i++) {
        const mid = (lo + hi) / 2;
        h1.style.fontSize = mid + 'px';
        if (h1.scrollHeight <= rightH) lo = mid;
        else hi = mid;
      }
      // Step 2: now verify no single line overflows the column width.
      // Measure actual rendered line width using a range on each text node.
      // Simple approach: if scrollWidth > clientWidth, scale down proportionally.
      h1.style.fontSize = Math.floor(lo) + 'px';
      // Allow layout to settle then check overflow
