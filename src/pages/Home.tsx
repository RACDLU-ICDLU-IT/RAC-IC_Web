...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const syncHeight = () => {
      ...
      // Max font-size: largest single word must fit column width.
      // Estimate: longest word chars * 0.62em per char at weight 900.
      const text = h1.textContent || '';
      const longestWord = text.split(/[\s\n]+/).reduce((a, b) => a.length > b.length ? a : b, '');
      const charCount = longestWord.length || 8;
      // 0.62 = approximate em width ratio for heavy sans at -0.04em tracking
      const maxByWidth = Math.floor(leftW / (charCount * 0.62));

      // Binary-search height fit, capped by width constraint
      let lo = 24, hi = Math.min(maxByWidth, 180);
      h1.style.fontSize = '';
      for (let i = 0; i < 22; i++) {
        const mid = (lo + hi) / 2;
