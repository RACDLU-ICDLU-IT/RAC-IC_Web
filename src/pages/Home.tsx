...
if (typeof document !== 'undefined' && !document.getElementById(INJECTED_ID)) {
  ...
  s.textContent = `
    ...
    .hv2-hero__inner {
      ...
      padding: 80px 40px;
      display: grid;
      /* Left col slightly wider to give headline more room */
    ...
    /* Left column: fills full grid cell height */
    .hv2-hero__left {
      display: flex;
      flex-direction: column;
      justify-content: center;
    ...
    /* Right column: logo + card grouped together, centred vertically */
    .hv2-hero__right {
      display: flex;
      flex-direction: column;
      justify-content: center;
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const syncHeight = () => {
      ...
      // Measure the RIGHT column's CONTENT height (scroll height of its children),
      // not its box height which equals the full grid row / 100svh.
      let contentH = 0;
      Array.from(rightEl.children).forEach((child) => {
        contentH += (child as HTMLElement).getBoundingClientRect().height;
      });
      // Add gap between logo and card (margin-bottom on logo-wrap = 16px)
      contentH += 16;
      if (contentH <= 0) return;

      // Binary-search: largest font-size where h1 natural height <= contentH
      h1.style.fontSize = '';
      let lo = 24, hi = 300;
      for (let i = 0; i < 25; i++) {
        const mid = (lo + hi) / 2;
        h1.style.fontSize = mid + 'px';
        if (h1.scrollHeight <= contentH) lo = mid;
        else hi = mid;
      }
      h1.style.fontSize = Math.floor(lo) + 'px';

      // Step 2: if any word overflows column width, scale down proportionally
      requestAnimationFrame(() => {
        if (h1.scrollWidth > leftW + 2) {
          const ratio = leftW / h1.scrollWidth;
          h1.style.fontSize = Math.floor(lo * ratio * 0.97) + 'px';
        }
      });
    };
