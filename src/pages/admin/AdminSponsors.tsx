...
export default function AdminSponsors() {
  ...
  const fetchSponsors = async () => {
    setLoading(true);
    try {
      const { data: snap } = await supabase.from('sponsors').select('*').eq('tenant_id', tenant.id);
      setSponsors(snap || []);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async () => {
    ...
    try {
      await supabase.from('sponsors').upsert({ id: docId, tenant_id: tenant.id, ...{ ...formData, createdAt: formData.createdAt || new Date().toISOString() } }, { onConflict: 'id' });
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await supabase.from('sponsors').delete().eq('id', deleteId).eq('tenant_id', tenant.id);
                      ...
                      {st.map(s => (
                         <div key={s.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-col group relative overflow-hidden transition-all hover:shadow-md">
                            ...
                            <div className={`w-full ${cardSize} flex items-center justify-center mb-3 bg-gray-50 rounded-lg p-2`}>
                               {s.logoUrl ? <img src={s.logoUrl} onError={(e) => { (e.target as HTMLImageElement).style.display='none'; }} className="max-w-full max-h-full object-contain mix-blend-multiply" /> : <span className="text-gray-300">No Logo</span>}
                            </div>
