...
export default function AdminBotManager() {
  ...
  const [filter, setFilter] = useState<'all' | 'human' | 'paused' | 'active'>('all');
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      // Auto-pause + clear needs_human when admin replies from web
      const resumeAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();
      await supabase.from('bot_paused').upsert(
        { psid: selectedPsid, page_id: pageId, paused_at: new Date().toISOString(), auto_resume_at: resumeAt, needs_human: false, reason: null },
        { onConflict: 'psid,page_id' }
      );
      await supabase.from('bot_conversations').insert({ psid: selectedPsid, page_id: pageId, role: 'admin', content: replyText.trim() });
      setConvos(prev => prev.map(c => c.psid === selectedPsid ? { ...c, paused: true, needs_human: false } : c));
      setReplyText('');
  ...
  const filteredConvos = convos.filter(c => {
    ...
    if (filter === 'paused') return c.paused && !c.needs_human;
            ...
            <div className="flex flex-col md:flex-row gap-3" style={{ height: '560px' }}>
              ...
              <div className={`${showChat ? 'hidden md:flex' : 'flex'} flex-col w-full md:w-64 shrink-0 border border-gray-200 rounded-xl overflow-hidden bg-gray-50`}>
                <div className="px-3 py-2.5 border-b border-gray-200 bg-white flex items-center justify-between">
                  ...
                  <span className="text-[11px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full font-bold">{filteredConvos.length}/{convos.length}</span>
                </div>
                {/* Filter bar */}
                <div className="flex border-b border-gray-100 bg-gray-50 shrink-0">
                  {([
                    { id: 'all', label: 'All', color: '' },
                    { id: 'human', label: '🚨 Human', color: 'red' },
                    { id: 'paused', label: '⏸ Paused', color: '' },
                    { id: 'active', label: '✓ Active', color: '' },
                  ] as const).map(f => (
                    <button key={f.id} onClick={() => setFilter(f.id as any)}
                      className={`flex-1 py-1.5 text-[9px] font-bold uppercase transition-colors border-b-2 ${
                        filter === f.id
                          ? f.id === 'human'
                ...
                <div className="overflow-y-auto flex-1">
                  {loadingConvos ? (
                    ...
                    <div className="p-6 text-center text-gray-400 text-xs">No conversations</div>
