...
/** Theme-aware colors, back in sync with Table.tsx now that it manages
 * its own light/dark background. The 'verified' badge reuses the
...
function paymentStatusMeta(status: PaymentStatus | undefined, p: ReturnType<typeof getClubPalette>, dark: boolean) {
  switch (status) {
    ...
    default:
      return { label: 'Unpaid', color: dark ? p.tsub : '#6b7280', bg: dark ? 'rgba(156,163,175,0.12)' : 'rgba(107,114,128,0.1)' };
  }
}

/** Human-readable one-liner for what happened in a single audit_log row,
 * tailored to the action/changes shapes this page actually writes. Falls
 * back to a generic "<action> on <table_name>" for anything unrecognized.
 * Does NOT include who did it — see describeLogActor below, which is
 * combined with this at render time so both log views (per-application
 * and page-level) share one source of truth for the "what" half. */
function describeLogAction(entry: any): string {
  const action = entry.changes?.action;
  switch (action) {
    case 'application_approved':
      return 'approved the application';
    case 'application_rejected':
      return `rejected the application${entry.changes?.note ? ` — "${entry.changes.note}"` : ''}`;
    case 'application_set_pending':
      return 'reset the application to pending';
    case 'application_deleted':
      return 'deleted the application';
    case 'payment_verified':
      return 'verified the payment';
    case 'payment_rejected':
      return `rejected the payment${entry.changes?.note ? ` — "${entry.changes.note}"` : ''}`;
    case 'payment_manual_update':
      return `manually set payment to "${entry.changes?.new_status}"${entry.changes?.note ? ` — "${entry.changes.note}"` : ''}`;
    case 'reconsideration_resolved':
      return 'marked the reconsideration request as resolved';
    case 'reconsideration_reopened':
      return 'marked the reconsideration request as unresolved';
    default:
      return `${entry.action} on ${entry.table_name}`;
  }
}

/** Resolves an audit_log row's actor_id to a display name using the
 * actorNames lookup map (built from the `users` table — see
 * fetchActorNames). Falls back to a truncated uuid if the actor no
 * longer resolves (e.g. the user account was later deleted), and to
 * "System" if actor_id is null (automated/background actions, if any
 * are ever logged this way). */
function describeLogActor(entry: any, actorNames: Record<string, string>): string {
  if (!entry.actor_id) return 'System';
  return actorNames[entry.actor_id] || `User (${String(entry.actor_id).slice(0, 8)}…)`;
}
...
export default function AdminApplications() {
  ...
  // Payment date — maps to payment_submitted_at. For the automatic flow
  // (applicant submits, admin verifies/rejects from pending_verification)
  // this is already stamped when the applicant submits and is never
  // touched here. For a manual override, admin is often recording a
  // payment that never went through the app at all (cash, phone-
  // confirmed bKash), so the date needs to be something admin can type
  // in directly rather than always defaulting to "now". Stored as a
  // <input type="datetime-local"> value string (no timezone suffix);
  // converted to an ISO string on save.
  ...
  // Log tab (per-application)
  const [logEntries, setLogEntries] = useState<any[]>([]);
  const [logLoading, setLogLoading] = useState(false);

  // Actor name resolution — shared by both the per-application log tab
  // and the page-level activity log below. Keyed by user id (uuid) so
  // repeated lookups across many log rows are O(1) instead of joining
  // per-row. Grows additively as more actor_ids are encountered across
  // either log view, never reset, so switching between views doesn't
  // re-fetch names already resolved.
  const [actorNames, setActorNames] = useState<Record<string, string>>({});

  // Page-level Activity Log — every applications-related audit_log row
  // for this tenant, not scoped to a single applicant. Separate from
  // the per-application log tab above (that one filters by record_id;
  // this one only filters by tenant_id + table_name).
  //
  // Paginated: first page loads 100 rows; each "Load More" press loads
  // 50 more via .range(), appended to what's already shown. loadingMore
  // is tracked separately from the initial activityLogLoading so the
  // existing list stays visible (not replaced by a full loading state)
  // while more rows come in.
  const ACTIVITY_LOG_INITIAL_LIMIT = 100;
  const ACTIVITY_LOG_PAGE_SIZE = 50;
  const [isActivityLogOpen, setIsActivityLogOpen] = useState(false);
  const [activityLogEntries, setActivityLogEntries] = useState<any[]>([]);
  ...
  // record_id -> applicant display label ("Name (email)"), so a mixed
  // tenant-wide feed can show which applicant each row was about.
  const [activityLogAppLabels, setActivityLogAppLabels] = useState<Record<string, string>>({});
  ...
  // Membership Conditions & Dues text — per-tenant, shown to first-time
  // applicants on the Join form before the blank application loads (see
  // Join.tsx's 'conditions' step). Stored on dues_settings alongside
  // application_fee / default_bkash_number. Empty/blank means the gate
  // is skipped entirely on the Join form.
  ...
  const loadConditionsText = async () => {
    try {
      const s: DuesSettings = await fetchDuesSettings();
      setConditionsText(s.membership_conditions_text || '');
    } catch (err) {
      console.error('loadConditionsText error:', err);
    } finally {
  ...
  // Admin-side trigger for the 72h (48h primary + 24h grace) unpaid-
  // application auto-expiry — see expire_unpaid_applications RPC. Since
  // there's no background/cron job in this build, cancellation is
  // triggered opportunistically from two places: here (whenever an
  // admin loads this page) and client-side in Join.tsx (whenever an
  // applicant re-checks their code). Both call the same RPC, which only
  // ever touches rows matching the exact stale/unpaid criteria — safe
  // to call redundantly from either side. Run before fetchApplications
  // so any application that just crossed the 72h mark shows as rejected
  // in the very first render rather than needing a manual refresh.
  const runExpiryCheck = async () => {
    try {
      await supabase.rpc('expire_unpaid_applications', { p_tenant_id: tenant.id });
    } catch (err) {
      console.warn('[applications] expire_unpaid_applications failed', err);
    }
  };
  ...
  // Resolves a batch of actor_ids against `users` (id, name) and merges
  // any newly-resolved names into actorNames. Only queries for ids not
  // already in the map, so repeated calls across log views/refreshes
  // stay cheap. Shared by both the per-application log tab and the
  // page-level activity log.
  const resolveActorNames = async (actorIds: (string | null | undefined)[]) => {
    const uniqueIds = Array.from(new Set(actorIds.filter((id): id is string => !!id)));
    const unresolved = uniqueIds.filter((id) => !(id in actorNames));
    if (unresolved.length === 0) return;
    try {
      const { data, error } = await supabase.from('users').select('id, name').in('id', unresolved);
      if (error) throw error;
      const next: Record<string, string> = {};
      (data || []).forEach((u: any) => {
        next[u.id] = u.name || 'Unnamed User';
      });
      // Any id that still didn't resolve (e.g. deleted user) gets a
      // placeholder cached too, so we don't re-query it every render.
      unresolved.forEach((id) => {
        if (!(id in next)) next[id] = '';
      });
      setActorNames((prev) => ({ ...prev, ...next }));
    } catch (err) {
      console.error('resolveActorNames error:', err);
    }
  };
  ...
  const fetchApplicationLog = async (appId: string) => {
    ...
    try {
      ...
      resolveActorNames(entries.map((e: any) => e.actor_id));
    } catch (err) {
  ...
  // Resolves applicant labels ("Name (email)") for a batch of
  // record_ids, merging results into activityLogAppLabels. Prefers the
  // already-loaded `applications` state (no extra query needed for
  // anything currently in the table), and only queries for record_ids
  // not found there (e.g. an application deleted after being logged).
  // Shared by both the initial activity log load and each "load more"
  // page, so labels accumulate rather than being re-fetched each time.
  const resolveApplicantLabels = async (entries: any[]) => {
    const recordIds = Array.from(new Set(entries.map((e: any) => e.record_id).filter(Boolean)));
    const labels: Record<string, string> = {};
    const missing: string[] = [];
    recordIds.forEach((id) => {
      if (id in activityLogAppLabels) return; // already resolved from a previous page
      const known = applications.find((a) => a.id === id);
      if (known) {
        labels[id] = `${known.name || 'Unknown'} (${known.email || 'no email'})`;
      } else {
        missing.push(id);
      }
    });
    if (missing.length > 0) {
      const { data: fallbackApps } = await supabase
        ...
        .in('id', missing)
        .eq('tenant_id', tenant.id);
      (fallbackApps || []).forEach((a: any) => {
        labels[a.id] = `${a.name || 'Unknown'} (${a.email || 'no email'})`;
      });
      // Anything still missing (application fully deleted, no
      // recoverable snapshot) falls back to a generic label rather
      // than a raw uuid in the UI.
      missing.forEach((id) => {
        if (!(id in labels)) labels[id] = 'Deleted Application';
      });
    }
    if (Object.keys(labels).length > 0) {
      setActivityLogAppLabels((prev) => ({ ...prev, ...labels }));
    }
  };

  // Page-level Activity Log — every applications audit_log row for this
  // tenant. Initial load fetches the most recent 100 rows (page 0).
  // Also resolves actor names (shared cache with the per-application
  // log) and applicant labels so a mixed feed spanning many applicants
  // is still readable at a glance.
  const fetchActivityLog = async () => {
    setActivityLogLoading(true);
    setActivityLogHasMore(true);
    try {
      const { data, error } = await supabase
        .from('audit_log')
        .select('*')
        ...
        .eq('table_name', 'applications')
        .order('created_at', { ascending: false })
        .range(0, ACTIVITY_LOG_INITIAL_LIMIT - 1);
      if (error) throw error;
      const entries = data || [];
      setActivityLogEntries(entries);
      setActivityLogHasMore(entries.length === ACTIVITY_LOG_INITIAL_LIMIT);
      resolveActorNames(entries.map((e: any) => e.actor_id));
      await resolveApplicantLabels(entries);
    } catch (err) {
  ...
  // "Load More" — fetches the next ACTIVITY_LOG_PAGE_SIZE (50) rows
  // starting right after what's currently loaded, via .range(), and
  // appends them to the existing list rather than replacing it. Using
  // .range() with a computed offset (not an ever-growing .limit())
  // means each page is a genuinely new slice of rows, not a re-fetch
  // of everything seen so far.
  const loadMoreActivityLog = async () => {
    setActivityLogLoadingMore(true);
    try {
      const from = activityLogEntries.length;
      ...
      if (error) throw error;
      const entries = data || [];
      setActivityLogEntries((prev) => [...prev, ...entries]);
      setActivityLogHasMore(entries.length === ACTIVITY_LOG_PAGE_SIZE);
      resolveActorNames(entries.map((e: any) => e.actor_id));
      await resolveApplicantLabels(entries);
    } catch (err) {
      console.error('loadMoreActivityLog error:', err);
  ...
  useEffect(() => {
    if (isActivityLogOpen) fetchActivityLog();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isActivityLogOpen]);
  ...
  // <input type="datetime-local"> works in the browser's local time with
  // no timezone info in the value string, so these two helpers handle
  // the round trip: ISO (from the DB) -> local datetime-local value (for
  // the input), and local datetime-local value -> ISO (for saving).
  ...
  const openManualPayment = (app: any) => {
    ...
    // Pre-fill from whatever's already on file (payment_submitted_at),
    // so editing an existing manual entry doesn't blank the date out.
    // Left empty for a fresh application with nothing submitted yet —
    // admin must actively choose a date rather than silently getting
    // "now" for a payment that may have happened days ago.
    setManualPaymentDate(isoToDatetimeLocal(app.payment_submitted_at));
  };

  const handleManualPaymentSave = async () => {
    if (!manualPaymentApp) return;
    if ((manualStatus === 'verified' || manualStatus === 'pending_verification' || manualStatus === 'rejected') && !manualPaymentDate) {
      addToast('Please enter the payment date.', 'error');
      return;
    }
    setManualSaving(true);
    try {
      const enteredDateIso = datetimeLocalToIso(manualPaymentDate);
      const patch: Record<string, any> = {
        ...
        // payment_submitted_at IS "payment date" for the manual-override
        // path — admin-entered, not auto-stamped to "now". The automatic
        // flow (applicant submits via the Join form) sets this field on
        // its own submission path and never goes through here.
        payment_submitted_at: enteredDateIso,
      };
  ...
  const handleSaveConditionsText = async () => {
    setConditionsSaving(true);
    try {
      // No validation on emptiness — a blank value is a valid,
      // intentional state (it means the Join form's conditions gate is
      // skipped entirely), not an error to reject like the fee amount's
      // numeric check above.
      const ok = await updateDuesSettings({
        membership_conditions_text: conditionsText.trim() || null,
      });
      if (ok) {
        addToast('Membership conditions saved.', 'success');
        setIsConditionsModalOpen(false);
      }
    } catch (err) {
      console.error(err);
      addToast('Failed to save membership conditions', 'error');
    } finally {
  ...
  const exportApplicationsExcel = (dataToExport: any[]) => {
    ...
    const rows = dataToExport.map((app: any) => ({
      ...
      'Payment Date': app.payment_submitted_at ? new Date(app.payment_submitted_at).toLocaleString() : '',
      'Photo URL': app.photo || '',
  ...
  return (
    <div className="rac-admin-applications">
          ...
          <div style={{ marginBottom: 12 }}>
            <Table
              ...
              renderRow={(app, i) => {
                ...
                // p.tl/p.tsub are fixed colors meant for the p.dark card
                // (used in dark mode), while p.td/p.mut are meant for the
                // p.lightCard card (used in light mode). Table.tsx switches
                // its card background between the two based on `dark`, so
                // row content here must follow the same switch rather than
                // using a flat p.tl (which is #eee in BOTH modes per
                // racPalette.ts — invisible against p.lightCard's pale tint).
                const rowText = dark ? p.tl : p.td;
                const rowSubText = dark ? p.tsub : p.mut;
                return (
                  <tr key={app.id} style={{ background: rowSelected ? p.greenDeep : 'transparent', borderBottom: `1px solid ${dark ? p.border : 'rgba(0,0,0,0.06)'}` }}>
                    ...
                    <td className="px-6 py-4">
                      <div style={{ fontWeight: 600, color: rowText, fontSize: 13 }}>{app.name}</div>
                      <div style={{ fontSize: 11, color: rowSubText }}>{app.email}</div>
                      {app.reference_number && (
                        <div style={{ fontSize: 10, color: rowSubText, fontFamily: 'monospace', marginTop: 1 }}>
                      ...
                      {status === 'rejected' && app.reconsideration_requested && (
                        <div
                          style={{
                            ...
                            fontSize: 9.5,
                            fontWeight: 700,
                            color: app.reconsideration_resolved ? rowSubText : '#e08a72',
                            marginTop: 3,
                            padding: '2px 7px',
                            borderRadius: 999,
                            background: app.reconsideration_resolved ? (dark ? 'rgba(156,163,175,0.14)' : 'rgba(107,114,128,0.1)') : 'rgba(224,138,114,0.14)',
                          }}
                        >
                    ...
                    <td className="px-6 py-4">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12.5, color: rowText }}>
                        <span>{age}</span>
                        {validAge ? (
                    ...
                    <td className="px-6 py-4" style={{ fontSize: 12, color: rowSubText }}>
          ...
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                Payment Date{manualStatus !== 'unpaid' && <span className="text-red-500"> *</span>}
              </label>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/50"
                />
                <button
                  type="button"
                  onClick={() => setManualPaymentDate(isoToDatetimeLocal(new Date().toISOString()))}
                  className="px-3 py-2 text-xs font-semibold text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors whitespace-nowrap"
                >
                  Now
                </button>
              </div>
              <p className="text-xs text-gray-400 mt-1.5">
                When the payment actually happened — not necessarily when you're entering this.
              </p>
            </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-2">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-sm">
                        {selectedApp.payment_submitted_at ? formatLogTimestamp(selectedApp.payment_submitted_at) : '-'}
                      </p>
                    </div>
                  </div>
                      ...
                      <div key={entry.id} className="flex gap-3 pb-3 border-b border-gray-50 last:border-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-sm text-gray-800">
                            <span className="font-semibold">{describeLogActor(entry, actorNames)}</span>{' '}
                            {describeLogAction(entry)}
                          </p>
      ...
      {/* ---------------- page-level activity log (tenant-wide, all applicants) ---------------- */}
      <Modal
        isOpen={isActivityLogOpen}
        onClose={() => setIsActivityLogOpen(false)}
        title="Applications Activity Log"
        size="lg"
      >
        <div className="space-y-4">
          <p className="text-xs text-gray-400">
            Every action taken on any application for this club, most recent first.
          </p>
          {activityLogLoading ? (
            <div className="text-center py-12 text-gray-400 text-sm">Loading activity log…</div>
                  ...
                  <div key={entry.id} className="flex gap-3 pb-3 border-b border-gray-50 last:border-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        <span className="text-xs text-gray-400">{formatLogTimestamp(entry.created_at)}</span>
                        {entry.record_id && (
                          <>
                            <span className="text-gray-300">·</span>
                            <span className="text-xs text-gray-500 truncate max-w-[220px]">
                              {activityLogAppLabels[entry.record_id] || 'Loading…'}
                            </span>
                          </>
                        )}
                      </div>
              ...
              {activityLogHasMore && (
                <div className="pt-1 flex justify-center">
                  <button
                    ...
                    disabled={activityLogLoadingMore}
                    className="flex items-center gap-2 text-sm font-semibold text-gray-600 hover:text-gray-900 disabled:opacity-50 border border-gray-200 hover:border-gray-300 rounded-lg px-4 py-2 transition-colors"
                  >
      ...
      <Modal
        ...
        title="Membership Conditions & Dues"
        size="md"
      >
        {!conditionsInitialized ? (
          ...
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Shown to first-time applicants on the Join form, right before the blank application
              loads. They must tick a checkbox agreeing to this text before continuing. Shown only
              once per applicant — not shown again when checking status or editing an existing
              application.
            </p>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                Conditions & Dues Text
              </label>
              <textarea
                value={conditionsText}
                onChange={(e) => setConditionsText(e.target.value)}
                rows={12}
                placeholder="e.g. Membership requires attending at least 3 meetings per Rotary year, paying annual dues of ৳XXX, and upholding the club's code of conduct…"
                className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/50 leading-relaxed resize-y"
              />
              <p className="text-xs text-gray-400 mt-1.5">
                {conditionsText.trim().length === 0
                  ? 'Empty — the conditions screen will be skipped entirely and applicants will go straight to the form.'
                  : `${conditionsText.trim().length.toLocaleString()} characters. Line breaks are preserved as written.`}
              </p>
            </div>

            <div className="pt-4 border-t border-gray-100 flex justify-end gap-3">
              <Button variant="secondary" onClick={() => setIsConditionsModalOpen(false)}>
                Cancel
              </Button>
              <Button onClick={handleSaveConditionsText} disabled={conditionsSaving}>
