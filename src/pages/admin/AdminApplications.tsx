...
/** Fixed, light-safe colors — this badge is rendered inside Table.tsx's
 * row cells, which sit on a permanently white/light background
 * (Table.tsx hardcodes bg-white regardless of theme). Using the
 * theme-flipping p.green/p.greenDeep palette here was the root cause
 * of the light-mode contrast bug: those colors are tuned for the
 * page's own dark summary cards, not for Table.tsx's fixed-light rows. */
function paymentStatusMeta(status: PaymentStatus | undefined) {
  switch (status) {
    case 'verified':
    ...
    default:
      return { label: 'Unpaid', color: '#6b7280', bg: '#f3f4f6' };
  }
}
          ...
          <div style={{ marginBottom: 12 }}>
            <Table
              ...
              renderRow={(app, i) => {
                ...
                const payMeta = paymentStatusMeta(app.payment_status);
                const status = (app.status || 'pending').toLowerCase();
                const rowSelected = selectedAppIds.includes(app.id);
                return (
                  <tr key={app.id} style={{ background: rowSelected ? '#fdf2f6' : 'transparent' }}>
                    ...
                    <td className="px-6 py-4">
                      <div style={{ fontWeight: 600, color: '#111827', fontSize: 13 }}>{app.name}</div>
                      <div style={{ fontSize: 11, color: '#6b7280' }}>{app.email}</div>
                      ...
                      {status === 'rejected' && app.reconsideration_requested && (
                        <div
                          style={{
                            ...
                            fontSize: 9.5,
                            fontWeight: 700,
                            color: app.reconsideration_resolved ? '#6b7280' : '#dc2626',
                            marginTop: 3,
                            padding: '2px 7px',
                            borderRadius: 999,
                            background: app.reconsideration_resolved ? '#f3f4f6' : '#fef2f2',
                          }}
                        >
                    ...
                    <td className="px-6 py-4">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12.5, color: '#111827' }}>
                        <span>{age}</span>
                        {validAge ? (
                    ...
                    <td className="px-6 py-4" style={{ fontSize: 12, color: '#6b7280' }}>
                    ...
                    <td className="px-6 py-4">
                      <span
                        style={{
                          ...
                          fontSize: 11,
                          fontWeight: 700,
                          background: status === 'approved' ? '#dcfce7' : status === 'rejected' ? '#fef2f2' : '#fef3c7',
