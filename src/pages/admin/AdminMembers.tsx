...
import { Users, Pencil, Trash, Eye, Download, KeyRound, Plus, Search, BadgeCheck } from 'lucide-react';
...
/**
 * ------------------------------------------------------------------
 * Visual identity — matches DashboardHome.tsx / DashboardProfile.tsx
 * exactly: same Inter font loader, same `!important` font-scoping
 * opt-out of the tenant theme system, same getClubPalette(tenant.id,
 * mode) hardcoded palette, same 20px-radius card language, same
 * p.dark/p.lightCard surface alternation, same page-top "label + live
 * clock" header row, same loading-skeleton pattern (role="status",
 * aria-busy, pulsing p.dark blocks at matching radius/spacing). This
 * revision brings AdminMembers fully in line with those two pages —
 * previously it had its own ad hoc page-top row (title + tenant pill,
 * no clock) and its own skeleton shape; both are now the same
 * component-level pattern reused verbatim.
 *
 * Only the p.* keys DashboardHome.tsx/DashboardProfile.tsx themselves
 * use are reused here (p.bg, p.dark, p.lightCard, p.border, p.tl,
 * p.tsub, p.tmid, p.td, p.mut, p.ptxt, p.green, p.greenDeep, p.av2,
 * p.dots, p.pillBorder, p.recBd, p.recTx) — no new palette keys
 * invented.
 *
 * CHANGELOG (this revision):
 *   - Page-top row now matches DashboardProfile/DashboardHome exactly:
 *     "Members" label + live clock (formatClock(), 30s refresh), with
 *     the tenant-id pill folded into the filter bar instead of the
 *     header row, since neither reference page's header row carries a
 *     second badge next to the title.
 *   - Loading skeleton now uses the same role/aria-busy pattern and
 *     block shapes as the two reference pages, instead of a bespoke
 *     3-block layout.
 *   - Added `rotaryMemberId` (official Rotary/Interact International
 *     ID) to the add/edit member form, the members table (shown under
 *     the member's memberId, matching the two-line meta treatment
 *     already used for name+email), the member detail view modal, and
 *     the Excel export. Distinct from `memberId` (internally
 *     generated, unchanged) — this field is plain free text with no
 *     generation logic, editable by admins and by the member
 *     themselves in DashboardProfile.
 * ------------------------------------------------------------------
 */
 ...
 * DashboardHome/DashboardProfile's formatClock(), reused verbatim so
 * the page-top header stays consistent across every page in the shell. */
 ...
 *
 * This is entirely separate from rotaryMemberId — see that field's
 * comments below. memberId generation is untouched by this revision.
 */
...
/** Fields that count toward a member's profile-completion % — kept in
...
export default function AdminMembers() {
  ...
  // Live clock — matches DashboardHome/DashboardProfile's cadence
  // (refreshed every 30s, not every second).
  ...
  const fetchMembers = async () => {
    ...
    setError(null);
    try {
      const [{ data: snap, error: membersError }, { data: roleData, error: rolesError }] = await Promise.all([
   ...
   *
   * rotaryMemberId requires no generation step — it's a plain optional
   * text field carried through in `data` like any other form field.
   *
  ...
  const handleSave = async (dataOverride?: any) => {
    ...
    try {
      ...
      console.error(err);
      // Surfaces a Postgres unique-violation on rotaryMemberId (see
      // migration.sql's users_tenant_rotary_member_id_unique index) as
      // a specific, actionable message instead of the raw DB error.
      if (err?.code === '23505' && String(err?.message || '').toLowerCase().includes('rotary')) {
        addToast('That Rotary Member ID is already in use in this club.', 'error');
      } else {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const { error } = await supabase.from('users').delete().eq('id', deleteId).eq('tenant_id', tenant.id);
  ...
  /* ------------------------------- styling tokens (mirrors DashboardHome/DashboardProfile) ------------------------------- */
      ...
      <div style={{ background: p.bg, padding: 18, transition: 'background .25s' }} className="p-4 md:p-8 -m-4 md:-m-8">
        <div style={{ maxWidth: 1080, margin: '0 auto', paddingBottom: 90 }}>
          ...
          {/* ---------------- page-top: title + live clock ----------------
              Matches DashboardHome/DashboardProfile's page-top row
              exactly (label + clock, same sizes/weights/spacing). The
              tenant-id pill moved into the filter bar below, since
              neither reference page carries a second badge in this row. */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 12, padding: '0 2px', gap: 12 }}>
            <span style={{ fontSize: 19, fontWeight: 600, color: p.ptxt, letterSpacing: '-.2px', flexShrink: 0 }}>Members</span>
          ...
          {/* ---------------- filter bar (tenant pill + add button) ---------------- */}
          <div style={{ ...cardDark, padding: 14, marginBottom: 12, display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative', flex: '1 1 220px', minWidth: 180 }}>
              ...
              <input
                placeholder="Search members, member ID, Rotary ID..."
                value={search}
            ...
            <button type="button" style={outlineBtn} onClick={() => exportExcel(filteredMembers)}>
              ...
              <table>
                ...
                <tbody>
                  {filteredMembers.length === 0 && (
                    <tr>
                      <td colSpan={10} style={{ textAlign: 'center', padding: '48px 16px', color: p.tsub }}>
                  ...
                  {filteredMembers.map(m => {
                    ...
                    return (
                      <tr key={m.id} style={selectedIds.includes(m.id) ? { background: p.greenDeep } : undefined}>
                        ...
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 74 }}>
                            <div style={{ flex: 1, height: 5, background: p.border, borderRadius: 4, overflow: 'hidden' }}>
                              <div style={{ height: '100%', width: `${pct}%`, background: p.green, borderRadius: 4 }} />
                            </div>
                            <span style={{ fontSize: 10.5, color: p.tsub, fontWeight: 600, flexShrink: 0 }}>{pct}%</span>
                          </div>
                        </td>
                        <td>
                          <button
                            onClick={async () => {
                              try {
                                const { error } = await supabase.from('users').update({ duesPaid: !m.duesPaid }).eq('id', m.id).eq('tenant_id', tenant.id);
                                if (error) throw error;
                                fetchMembers();
                              } catch (err) {
                                console.error(err);
                                addToast('Failed to update dues status', 'error');
                              }
                            }}
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={formData.id ? 'Edit member' : 'Add member'} size="lg">
        <div style={{ background: p.bg }} className="-m-4 md:-m-6 p-4 md:p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {formData.id && (
              <div>
                <label style={labelStyle}>Assigned member ID</label>
                <input value={formData.memberId || '—'} disabled style={disabledInputStyle} />
        ...
        <div style={{ background: p.bg }} className="-m-4 md:-m-6 p-4 md:p-6">
          {viewMember && (
            <div className="space-y-4" style={{ fontSize: 13 }}>
              ...
              {/* Profile completion — same COMPLETION_FIELDS as
                  DashboardProfile.tsx, so this % can never disagree with
                  what the member themselves sees. */}
