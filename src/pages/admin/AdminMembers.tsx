...
export default function AdminMembers() {
  ...
  // Only master admin can set roles now (role management moved to dedicated Roles page)
  ...
  return (
    <div className="space-y-8 pb-32">
      <div className="flex items-start justify-between">
        ...
        <Button onClick={() => {
          setFormData({ role_id: null, status: 'active', tenant_id: tenant.id, memberId: `${tenant.shortName.substring(0,2).toUpperCase()}-${new Date().getFullYear()}-${Math.floor(100 + Math.random() * 900)}` });
          setIsFormOpen(true);
        }}>
           Add Member
        </Button>
      ...
      <Table
        ...
        renderRow={(m) => {
          ...
          <tr key={m.id} className={selectedIds.includes(m.id) ? 'bg-primary/5' : ''}>
            ...
            <td className="px-6 py-4">
              <span className="inline-flex px-2 py-0.5 rounded text-xs font-bold uppercase text-white" style={{ background: mRole?.color || '#9ca3af' }}>
                {mRole?.label || 'Member'}
              </span>
            </td>
            <td className="px-6 py-4">
            ...
            <div className="flex gap-4 items-center">
              ...
              <div>
                ...
                <div className="mt-1 flex gap-2">
                  <span className="uppercase text-[10px] font-bold bg-gray-100 px-2 py-0.5 rounded">{roleById(viewMember.role_id)?.label || 'Member'}</span>
