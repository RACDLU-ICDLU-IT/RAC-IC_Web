...
/** NOTE: DB column is `created_at` (snake_case), not `createdAt`. Field
 * kept as createdAt on this client-side type for continuity with the
 * rest of this file, but it's populated from row.created_at at the
 * query boundary — see loadDashboardData(). */
 ...
 * NOTE: no funding fields exist here. When `projects` is empty, the
 * Active projects card keeps its original title-row + geometric-
 * placeholder structure (same as the mockup's static "Books for
 * Barisal" layout) with "No ongoing project available" in the title
 * slot instead of a fictional project name — see that card's JSX
 * below. The "On track" status toggle only renders when a real
 * project exists (there's no status to show for no project). The
 * funds-raised bar is omitted entirely even when a real project
 * exists, since no funding field exists on this table yet and it
 * only ever rendered a fabricated percentage. */
type ProjectRecord = {
  id: string;
  name: string;
  type?: string;
...
/** One row from `announcements`, trimmed to only what the "Needs
 * attention" card needs (title + enough to determine visibility and
 * ordering). Visibility/audience filtering (target_all /
 * target_roles / target_members) and the published + not-expired
 * check mirror DashboardAnnouncements.tsx's isVisibleToMember() /
 * isEffectivelyExpired() exactly, so this card can never show
 * something a member couldn't actually open on that page — see
 * isAnnouncementVisible() below. Kept as a narrow local type (not
 * importing AnnouncementRow from DashboardAnnouncements.tsx) since
 * only a handful of fields are needed here and the two pages don't
 * currently share a types module. */
type NeedsAttentionAnnouncement = {
  id: string;
  title: string;
  is_pinned: boolean;
  status: string;
...
/* ------------------------------- profile completion (mirrors DashboardProfile.tsx) ------------------------------- */

/** Identical field list + weighting to DashboardProfile.tsx's
 * COMPLETION_FIELDS — kept as an exact copy (not imported, the two
 * pages don't share a module) so the verified-badge threshold here
 ...
 * name (same idea as Meta/Instagram's blue check, scoped to this
 * club's own completeness bar instead of platform verification).
 *   - 'filled'  → profile.status === 'active' AND 100% complete
 *   - 'outline' → profile.status === 'active' AND 60%+ complete (but < 100%)
 *   - null      → inactive/pending/suspended status, OR < 60% complete
 * Status gating comes first and is absolute: an inactive/pending
 * member never gets a badge regardless of how complete their profile
 * fields are, since the badge is meant to read as "current, verified
 * club member," not just "filled out their form." */
type VerifiedTier = 'filled' | 'outline' | null;
function verifiedTierFor(status: string | undefined, pct: number): VerifiedTier {
...
/* ------------------------------- announcement visibility (mirrors DashboardAnnouncements.tsx) ------------------------------- */

/** Identical rule to DashboardAnnouncements.tsx's isEffectivelyExpired
 * — intentionally kept in lockstep so this card and the Announcements
 * page can never disagree about what counts as expired. */
function isAnnouncementEffectivelyExpired(a: Pick<NeedsAttentionAnnouncement, 'status' | 'expires_at' | 'is_permanent'>): boolean {
...
/** Identical rule to DashboardAnnouncements.tsx's isVisibleToMember —
 * published, not expired, and audience-matched (target_all, or this
 * member's role in target_roles, or this member's id in
 * target_members). Kept as its own copy (not imported) since the two
 * files don't currently share a module, but the logic must never
 * drift from the Announcements page's own definition of "visible to
 * this member" — otherwise this card could surface something the
 * member can't actually open. */
function isAnnouncementVisible(a: NeedsAttentionAnnouncement, memberId: string | undefined, roleId: string | null | undefined): boolean {
  if (a.status !== 'published') return false;
  if (isAnnouncementEffectivelyExpired(a)) return false;
...
async function loadDashboardData(tenantId: string, userId?: string) {
  ...
  // Local-date ISO formatter — deliberately NOT using Date.toISOString(),
  // which converts to UTC before formatting. For any timezone ahead of
  // UTC (e.g. Asia/Dhaka, UTC+6), that silently shifts a local date
  // backward by a day: local midnight Sunday July 19 becomes
  // "2026-07-18" once UTC-shifted. That one-day shift was the actual
  // bug behind the Tracking card's "upcoming this week" count — it
  // pushed endOfWeekISO to the day BEFORE today, so an event happening
  // today could never be counted as "this week" at all (while the
  // month boundary, shifted by the same bug but with more slack before
  // hitting a real edge, happened to still include it — hence week
  // showing 0 and month showing 1 for what should have been the same
  // single event). This mirrors the isoDate() helper already used in
  // DashboardAttendance.tsx / DashboardCalendar.tsx for the same
  // reason there.
  ...
  // Start-of-week (Sunday) and end-of-month boundaries, both as
  // local-midnight ISO dates via localISODate() above — NOT
  // toISOString(), for the reason explained there.
  const startOfWeekDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() - now.getDay());
  const startOfWeekISO = localISODate(startOfWeekDate);
  const endOfMonthISO = localISODate(new Date(now.getFullYear(), now.getMonth() + 1, 0));
  ...
  const [
    ...
    supabase
      ...
      .limit(4),
    // DB column is `created_at` (snake_case). Querying the camelCase
    // `createdAt` throws Postgres error 42703 ("column
    // announcements.createdAt does not exist"), which was surfacing
    // as "Couldn't load your dashboard data" for the whole page since
    // this query's error is thrown (see `if (annRes.error) throw`
    // below), aborting the entire Promise.all consumer in
    // fetchDashboard's try/catch.
    supabase
      .from('announcements')
      .select('*')
      .eq('tenant_id', tenantId)
    ...
    // Tracking card: every event in [start-of-week, end-of-month],
    // used to derive both the "this week" and "this month" upcoming
    // counts client-side (see fetchDashboard) rather than issuing two
    // separate range queries — the month range is always a superset
    // of the remaining-week range from `today` onward.
    supabase
      .from('events')
      .select('id, date')
      .eq('tenant_id', tenantId)
    ...
    // Needs attention card: full announcements rows (not the trimmed
    // `announcements` select above) so targeting/expiry can be
    // evaluated with the exact same fields DashboardAnnouncements.tsx
    // uses. Filtered to status=published at the query level as a
    // narrowing (mirrors that page's own query); expiry + audience
    // targeting is still evaluated client-side via
    // isAnnouncementVisible(), same as that page does.
    supabase
      .from('announcements')
      .select('id, title, is_pinned, status, expires_at, is_permanent, target_all, target_roles, target_members, created_at, published_at')
      .eq('tenant_id', tenantId)
      .eq('status', 'published')
      .order('created_at', { ascending: false }),
  ]);
  ...
  // announcement.createdAt (e.g. a future "posted Xh ago" via
  // timeAgo()) keeps working without having to know about the raw
  // column name.
  const announcements = ((annRes.data as any[]) || []).map((r) => ({
    ...r,
...
export default function DashboardHome() {
  ...
  // Tracking card — count of events in [today, end of this week] and
  // [today, end of this month]. Both computed from the same
  // upcomingRangeEvents fetch (see loadDashboardData) rather than two
  // separate queries.
  const [upcomingThisWeek, setUpcomingThisWeek] = useState(0);
  const [upcomingThisMonth, setUpcomingThisMonth] = useState(0);

  // Needs attention card — real published announcements this member
  // can see, pinned first then newest (same ordering rule as
  // DashboardAnnouncements.tsx), trimmed to the first 3 here.
  const [attentionAnnouncements, setAttentionAnnouncements] = useState<NeedsAttentionAnnouncement[]>([]);

  // Verified badge (welcome card) — needs this member's own `users`
  // row fields (status + every COMPLETION_FIELDS key), fetched the
  // same way DashboardProfile.tsx fetches its formData. Kept as its
  // own small piece of state/effect rather than folded into
  // loadDashboardData's Promise.all — this is a narrow, single-table
  // read for one card, not part of that function's broader
  // "dashboard data" shape.
  const [profileRow, setProfileRow] = useState<any>(null);

  // "Last 7 meetings" row selection — selectedReportDay actually holds
  // the tapped meeting's eventId (name kept for continuity with earlier
  // state), looked up directly against meetingDays rather than a
  // date-keyed map, since eventId is already unique per entry.
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      ...
      // Tracking card counts: "this week" = today..end of this week
      // (Saturday); "this month" = today..end of this month.
      // upcomingRangeEvents already spans [start-of-week, end-of-month],
      // so both counts are simple date-string comparisons against it
      // rather than separate round trips.
      const endOfWeekDate = new Date(data.startOfWeekISO + 'T00:00:00');
      ...
      const weekCount = data.upcomingRangeEvents.filter((e) => e.date >= data.todayISO && e.date <= endOfWeekISO).length;
      const monthCount = data.upcomingRangeEvents.filter((e) => e.date >= data.todayISO && e.date <= data.endOfMonthISO).length;
      setUpcomingThisWeek(weekCount);
      setUpcomingThisMonth(monthCount);

      // Needs attention card: filter to what this member can actually
      // see (same rule DashboardAnnouncements.tsx uses), pinned first
      // then newest, trimmed to 3 for the card.
      const roleId = (user as any)?.role_id ?? (role as any)?.id ?? null;
      const visibleAnn = data.needsAttentionAnnouncements
        .filter((a) => isAnnouncementVisible(a, user.id, roleId))
        .sort((a, b) => {
          if (a.is_pinned !== b.is_pinned) return a.is_pinned ? -1 : 1;
          return new Date(b.created_at).getTime() - new Date(a.created_at).getTime();
        })
        .slice(0, 3);
      setAttentionAnnouncements(visibleAnn);
    } catch (err) {
      console.error(err);
  ...
  // Verified badge data — this member's own `users` row (status +
  // every field computeCompletion() reads), fetched independently of
  // the main dashboard load above since it's a single narrow read for
  // one small piece of UI. Same table/columns DashboardProfile.tsx
  // reads for the identical purpose, so the badge and the Profile
  // page's own % can never see different underlying data.
  useEffect(() => {
    let cancelled = false;
    if (!user?.id) return;
    supabase
      ...
      .then(({ data, error: err }) => {
        if (cancelled) return;
        if (err) {
          console.warn('[dashboard] Verified-badge profile fetch failed:', err);
          return;
        }
        setProfileRow(data);
      });
  ...
  // Lookup for the "Last 7 meetings" row — selectedReportDay holds the
  ...
  // Verified badge tier for the welcome card. Computed from
  // profileRow (this member's own `users` row, fetched above) rather
  // than the lighter useAuth().profile object, since status +
  // completion both need to read from the same source
  ...
  return (
    <div className="rac-dashboard-home">
      ...
      <style>{`
        ...
        /* Needs attention -> announcement row links. Kept as its own
           class rather than inline hover styles since plain inline
           style attrs can't express :hover. Visuals otherwise match
           the card's previous static rows exactly (same font-size/
           color/line-height as the old "3 dues unpaid" / "TRF Chair
           unassigned" rows) — only the interactive affordance is new. */
        .rac-needs-attention-row {
          display: block;
      ...
      <div style={{ background: p.bg, padding: 18, transition: 'background .25s' }} className="p-4 md:p-8 -m-4 md:-m-8">
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          >
            <div className="rac-welcome-id">
              ...
              <div style={{ minWidth: 0 }}>
                ...
                >
                  <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{profile?.name || 'there'}</span>
                  {/* Verified badge — filled once status is active AND
                      the profile is 100% complete (see COMPLETION_FIELDS
                      above); outline once active AND 60%+ complete.
                      Inactive/pending members never get a badge
                      regardless of completion %, and members under 60%
                      complete get no badge regardless of status. */}
                  {verifiedTier === 'filled' && (
                    <BadgeCheck
                      size={16}
                      style={{ flexShrink: 0, color: p.green, fill: p.green, stroke: p.dark }}
                      aria-label="Verified: active member, profile complete"
                    />
                  )}
                  {verifiedTier === 'outline' && (
                    <BadgeCheck
                      size={16}
                      style={{ flexShrink: 0, color: p.green, fill: 'none' }}
                      aria-label="Verified: active member, profile mostly complete"
                    />
                  )}
                </div>
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.65fr 1fr 1fr', gap: 12, marginBottom: 12 }} className="!grid-cols-1 sm:!grid-cols-2 lg:!grid-cols-[1.65fr_1fr_1fr]">
            ...
            {/* Active projects */}
            <div style={{ borderRadius: 20, padding: 16, background: p.dark, color: p.tl, border: `1px solid ${p.border}` }}>
              {/* Title row: real project name when one exists, otherwise
                  "No ongoing project available" in the exact same slot
                  the mockup's static "Books for Barisal" occupied —
                  same structure as the populated state below, just
                  without the "On track" status toggle (there's no
                  project to have a status). This replaces an earlier
                  version of this empty state that broke out into its
                  own centered layout; that read as a different card
                  shape rather than the same card with nothing in it. */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 11, color: p.tmid, marginBottom: 11 }}>
                <span>{projects[0]?.name || 'No ongoing project available'}</span>
                {projects.length > 0 && (
              ...
              {/* Geometric placeholder — same SVG the populated state
                  uses when a project has no coverImage, kept here
                  unconditionally for the empty state too (there's
                  never a coverImage to show when there's no project). */}
              <div
                style={{
                  height: 108,
                  borderRadius: 12,
                  ...
                  marginBottom: projects.length > 0 ? 11 : 0,
                  position: 'relative',
                  overflow: 'hidden',
              ...
              {/* Funds raised bar intentionally omitted even when a
                  real project exists: no funding fields exist on the
                  real `projects` table (see ProjectRecord comment
                  above), so this row only ever rendered a hardcoded
                  83%/64% placeholder, not real data. Nothing here
                  should claim to be "funds raised" until a real
                  funding field exists on `projects`. */}
            </div>

            {/* Needs attention — now backed by real published
                announcements this member can see (see
                isAnnouncementVisible), pinned first then newest,
                showing up to 3 titles. Clicking a row opens the full
                announcement on the Announcements page. Card shell/
                padding/border/heading row are unchanged from before;
                only the two static rows are replaced. */}
            <div style={{ borderRadius: 20, padding: 16, background: p.dark, color: p.tl, border: `1px solid ${p.border}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <span style={{ fontSize: 13, fontWeight: 600 }}>Needs attention</span>
                <span style={{ color: p.dots, fontSize: 15, letterSpacing: 1 }}>···</span>
              </div>
              {attentionAnnouncements.length === 0 ? (
                <div style={{ fontSize: 10.5, color: p.tsub, textAlign: 'center', padding: '18px 4px' }}>No announcements right now.</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {attentionAnnouncements.map((a, i) => (
                    <Link
                      key={a.id}
                      to={`/dashboard/announcements?id=${a.id}`}
                      className="rac-needs-attention-row"
                      style={
                        i === 0
                          ? { background: p.greenDeep, border: `1px solid ${p.recBd}`, borderRadius: 12, padding: 10, marginBottom: attentionAnnouncements.length > 1 ? 4 : 0 }
                          : { padding: '8px 10px' }
                      }
                    >
                      <span
                        style={
                          i === 0
                            ? { color: p.av2, display: 'block', fontWeight: 600, marginBottom: 2, fontSize: 11, lineHeight: 1.45 }
                            : { display: 'block', color: p.tl, fontWeight: 600, fontSize: 11, lineHeight: 1.45 }
                        }
                      >
                        {a.is_pinned ? '📌 ' : ''}
                        {a.title}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '.66fr 1fr 1.32fr', gap: 12 }} className="!grid-cols-1 sm:!grid-cols-2 lg:!grid-cols-[.66fr_1fr_1.32fr]">
            {/* Tracking — now shows real upcoming-event counts for
                this week and this month (see loadDashboardData's
                upcomingRangeEvents fetch + the week/month count calc
                in fetchDashboard), replacing the old hardcoded
                "7 applications" placeholder. Card shell/heading row
                unchanged; only the two data rows below the heading
                are new. */}
            <div style={{ borderRadius: 20, padding: 16, background: p.lightCard, color: p.td }}>
              ...
              <div style={{ fontSize: 10.5, color: p.mut }}>Upcoming this week</div>
              <div style={{ fontSize: 24, fontWeight: 600, marginTop: 8, letterSpacing: '-.3px' }}>{upcomingThisWeek}</div>
              <div style={{ fontSize: 9.5, color: p.mut, marginTop: 2, marginBottom: 18 }}>{upcomingThisWeek === 1 ? 'event' : 'events'}</div>

              <div style={{ fontSize: 10.5, color: p.mut, paddingTop: 12, borderTop: `1px solid ${p.border}` }}>Upcoming this month</div>
              <div style={{ fontSize: 24, fontWeight: 600, marginTop: 8, letterSpacing: '-.3px' }}>{upcomingThisMonth}</div>
              <div style={{ fontSize: 9.5, color: p.mut, marginTop: 2 }}>{upcomingThisMonth === 1 ? 'event' : 'events'}</div>
            </div>

            {/* Detailed report — last 7 meetings, most recent first, one box
                each. meetingDays is already sorted desc by date, so this is
                just the first 7 entries — no meetings older than that are
                shown, and if there are fewer than 7 total, only that many
                boxes render (no blanks). Each box shows the date and the
                club-wide present count only; this member's own status is
                revealed only on tap, in the detail panel below the row. */}
            <div style={{ borderRadius: 20, padding: 16, background: p.dark, color: p.tl, border: `1px solid ${p.border}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <span style={{ fontSize: 13, fontWeight: 600 }}>Detailed report</span>
              </div>
              <div style={{ fontSize: 10.5, color: p.tsub, marginBottom: 11 }}>Last 7 meetings</div>

              {meetingDays.length === 0 ? (
                <div style={{ fontSize: 10.5, color: p.tsub, padding: '10px 0', textAlign: 'center' }}>No past meetings recorded yet.</div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(meetingDays.length, 7)}, 1fr)`, gap: 5 }}>
                  {meetingDays.slice(0, 7).map((d) => {
                    const isSelected = d.eventId === selectedReportDay;
                    return (
                      <button
                        key={d.eventId}
                        type="button"
                        onClick={() => setSelectedReportDay(isSelected ? null : d.eventId)}
                        style={{
                          borderRadius: 10,
                          border: `1px solid ${isSelected ? p.green : p.border}`,
                          background: isSelected ? p.greenDeep : p.lightCard,
                          cursor: 'pointer',
                          display: 'flex',
                      ...
                      >
                        <span style={{ fontSize: 8.5, color: isSelected ? '#e6b8c4' : p.tsub, fontWeight: 600, textAlign: 'center', lineHeight: 1.25 }}>
                          {formatShortDate(d.date)}
                        </span>
                        <span style={{ fontSize: 13, fontWeight: 700, color: isSelected ? '#ffffff' : p.tl }}>{d.presentCount}</span>
                        <span style={{ fontSize: 7, color: isSelected ? '#e6b8c4' : p.tsub }}>present</span>
                      </button>
                    );
                  })}
                </div>
              ...
              {selectedReportRow ? (
                <div style={{ marginTop: 12, background: p.lightCard, borderRadius: 12, padding: 10 }}>
                  <div style={{ fontSize: 10.5, color: p.mut, fontWeight: 600, marginBottom: 6 }}>{formatShortDate(selectedReportRow.date)}</div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 10.5, color: p.td }}>{selectedReportRow.presentCount} present</span>
                    <span style={{ fontSize: 9.5, fontWeight: 700, color: '#f2eef0', background: p.dark, padding: '3px 9px', borderRadius: 20 }}>
                      {selectedReportRow.myStatus === 'not_required'
                        ? 'N/A'
                        : timelineKeyForStatus(selectedReportRow.myStatus)
                        ? timelineKeyForStatus(selectedReportRow.myStatus)!.charAt(0).toUpperCase() + timelineKeyForStatus(selectedReportRow.myStatus)!.slice(1)
                        : selectedReportRow.myStatus || 'Not marked'}
                    </span>
                  </div>
                </div>
              ) : meetingDays.length > 0 ? (
                <div style={{ fontSize: 9.5, color: p.tsub, marginTop: 12, textAlign: 'center' }}>Tap a meeting to see your status.</div>
              ) : null}
            </div>
