...
export default function DashboardHome() {
  ...
  // Live clock — was present in the original mockup, dropped by mistake
  // during the React rebuild. Updates every 30s (not every second — this
  // is a page-header clock, not a stopwatch, and second-level re-renders
  ...
  return (
    <div className="rac-dashboard-home">
      ...
      >
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
        {/* ---------------- page-top: title + live clock + local theme toggle ---------------- */}
        <div
          style={{
            display: 'flex',
        ...
        >
          ...
          <span style={{ fontSize: 16, fontWeight: 600, color: p.ptxt, flex: 1, textAlign: 'right' }}>
            {clockLabel}
            <span style={{ fontSize: 10.5, color: p.pmut, fontWeight: 500, marginLeft: 6 }}>Time</span>
          </span>
          <button
            type="button"
            onClick={() => setDark((v) => !v)}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
              ...
              <div style={{ position: 'relative', width: ringSize, height: ringSize, flexShrink: 0 }}>
                <svg width={ringSize} height={ringSize} viewBox={`0 0 ${ringSize} ${ringSize}`} style={{ transform: 'rotate(-90deg)' }}>
                  {/* Track: was p.border (#292929, near-black) — far too bold against a
                      light card, which is exactly why an empty ring read as "broken" rather
                      than "no data yet". Now a faint tint of the card's own text color. */}
                  <circle
                    cx={ringSize / 2}
                    cy={ringSize / 2}
                    r={ringRadius}
                ...
                <div
                  style={{
                    ...
                    alignItems: 'center',
                    justifyContent: 'center',
                    fontSize: attendanceRate !== null ? 15 : 11,
                    fontWeight: 600,
                    color: attendanceRate !== null ? p.td : p.mut,
                  }}
                >
