 ...
 * executionDate, coverImage, tenant_id. Note the field is `name`, not `title`.
 */
type ProjectRecord = {
  id: string;
  name: string;
    ...
    <div style={tokens} className="space-y-6 sm:space-y-8 animate-fade-in-up antialiased">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="space-y-6">
          ...
          {projects.length > 0 && (
            <SectionCard hover className="p-5 sm:p-6">
              <SectionHeader icon={<FolderKanban size={18} />} title="Club Projects" to="/dashboard/projects" />
              <div className="space-y-2.5">
                {projects.map((project) => (
                  <Link
                    key={project.id}
                    to="/dashboard/projects"
                    className="flex items-center gap-3 p-3 rounded-2xl hover:bg-[color:var(--surface-hover)] border border-transparent hover:border-[color:var(--border-subtle)] transition-colors group"
                  >
                    <div className="w-10 h-10 rounded-xl overflow-hidden shrink-0 bg-gradient-to-br from-[color:var(--accent-soft)] to-[color:var(--surface)]">
                      {project.coverImage && (
                        <img src={project.coverImage} alt="" className="w-full h-full object-cover" />
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="font-bold text-sm text-[color:var(--text-1)] truncate">{project.name}</p>
                      <p className="text-xs text-[color:var(--text-3)] truncate">
                      ...
                      <span
                        className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shrink-0 ${
                          project.status.toLowerCase() === 'ongoing'
                            ...
                            ? 'text-[color:var(--text-3)] bg-[color:var(--surface-2)]'
