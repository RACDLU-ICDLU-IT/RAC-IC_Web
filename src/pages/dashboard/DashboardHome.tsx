                ...
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(meetingDays.length, 7)}, 1fr)`, gap: 5 }}>
                  {meetingDays.slice(0, 7).map((d) => {
                    ...
                    return (
                      ...
                      >
                        <span style={{ fontSize: 8.5, color: isSelected ? p.recTx : p.tsub, fontWeight: 600, textAlign: 'center', lineHeight: 1.25 }}>
                          {formatShortDate(d.date)}
                        </span>
                        <span style={{ fontSize: 13, fontWeight: 700, color: isSelected ? p.av2 : p.tl }}>{d.presentCount}</span>
                        <span style={{ fontSize: 7, color: isSelected ? p.recTx : p.tsub }}>present</span>
                      </button>
                    );
                  })}
                </div>
