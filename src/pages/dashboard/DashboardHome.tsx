 ...
 * Active projects card now shows an explicit "No ongoing project
 * available" state instead of falling back to a fictional "Books for
 * Barisal" placeholder + fake funds bar — see that card's JSX below.
 * When at least one real project exists, the funds-raised bar is
 * omitted entirely (rather than showing a fabricated percentage)
 * since no funding field exists on this table yet. */
type ProjectRecord = {
  id: string;
  name: string;
  type?: string;
...
async function loadDashboardData(tenantId: string, userId?: string) {
  ...
  // local-midnight ISO dates — same isoDate-by-hand approach the
  // Attendance/Calendar pages use elsewhere in this codebase (no extra
  // date library dependency introduced here).
      ...
      <div style={{ background: p.bg, padding: 18, transition: 'background .25s' }} className="p-4 md:p-8 -m-4 md:-m-8">
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.65fr 1fr 1fr', gap: 12, marginBottom: 12 }} className="!grid-cols-1 sm:!grid-cols-2 lg:!grid-cols-[1.65fr_1fr_1fr]">
            ...
            <div style={{ borderRadius: 20, padding: 16, background: p.dark, color: p.tl, border: `1px solid ${p.border}` }}>
              {projects.length === 0 ? (
                // No real projects — show an explicit empty state
                // instead of the mockup's fictional "Books for
                // Barisal" fallback + fake funds bar. Keeps the same
                // card shell/padding/border as the populated state so
                // layout doesn't shift between the two.
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', minHeight: 160, gap: 6 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 600, color: p.tl }}>No ongoing project available</div>
                  <div style={{ fontSize: 10.5, color: p.tsub }}>New projects will show up here once created.</div>
                </div>
              ) : (
                <>
                  ...
                  {/* Funds raised bar intentionally omitted: no funding
                      fields exist on the real `projects` table (see
                      ProjectRecord comment above), so this row only
                      ever rendered a hardcoded 83%/64% placeholder,
                      not real data. Nothing here should claim to be
                      "funds raised" until a real funding field exists
                      on `projects`. */}
                </>
              )}
            </div>
