...
/* ------------------------------- calendar grid helpers (Detailed report card) -------------------------------
 * Same shape as DashboardAttendance.tsx's buildMonthGrid/isoDate/todayISO —
 * duplicated here rather than imported since that file's grid is scoped to
 * the member's own attendance only, while this card needs club-wide
 * present counts per day (meetingDays), a different data shape entirely. */
...
/** Dot color for a meeting day's status, matching DashboardAttendance.tsx's
 * statusColor for the four XP-bearing statuses. 'not_required' and unmarked
 * days get no dot at all (handled at the call site). */
function meetingDotColor(p: any, key: TimelineKey) {
  if (key === 'present') return p.green;
  if (key === 'late') return p.av2;
  if (key === 'excused') return '#c9a45c';
  return '#e08a72'; // absent
}
...
export default function DashboardHome() {
  ...
  // Detailed report calendar grid — month being viewed, and which day (if
  // any) is tapped/selected to show its full row below the grid. Defaults
  // to the current calendar month on mount, same as DashboardAttendance.tsx.
  ...
  // Lookup map for the Detailed report calendar grid — one entry per
  // meeting date. Built here (not inside the JSX loop) so each grid cell
  // render is a plain map.get() rather than an array .find() per cell.
  const meetingDaysByDate = useMemo(() => {
    const map = new Map<string, MeetingDayRecord>();
    meetingDays.forEach((d) => map.set(d.date, d));
    return map;
  }, [meetingDays]);

  const reportCells = buildMonthGrid(reportMonth.y, reportMonth.m);
  const reportMonthLabel = new Date(reportMonth.y, reportMonth.m, 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
  const reportToday = todayISO();
  const selectedReportRow = selectedReportDay ? meetingDaysByDate.get(selectedReportDay) || null : null;
      ...
      <div style={{ background: p.bg, padding: 18, transition: 'background .25s' }} className="p-4 md:p-8 -m-4 md:-m-8">
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '.66fr 1fr 1.32fr', gap: 12 }} className="!grid-cols-1 sm:!grid-cols-2 lg:!grid-cols-[.66fr_1fr_1.32fr]">
            ...
            {/* Detailed report — real month calendar grid, same visual
                pattern as DashboardAttendance.tsx's calendar view (‹ Month ›
                header, 7-col day grid, status dot per marked day), driven by
                club-wide meetingDays instead of that file's member-only
                records. Each cell additionally shows the club-wide present
                count in tiny text under the dot; tapping a marked day opens
                a detail row below the grid with the full date + count +
                this member's own status. */}
            <div style={{ borderRadius: 20, padding: 16, background: p.dark, color: p.tl, border: `1px solid ${p.border}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <button
                  type="button"
                  onClick={() => { setReportMonth((c) => c.m === 0 ? { y: c.y - 1, m: 11 } : { y: c.y, m: c.m - 1 }); setSelectedReportDay(null); }}
                  style={{ border: `1px solid ${p.pillBorder}`, borderRadius: 20, padding: '5px 8px', color: p.tmid, background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                >
                  <ChevronLeft size={12} />
                </button>
                <span style={{ fontSize: 13, fontWeight: 600 }}>{reportMonthLabel}</span>
                <button
                  type="button"
                  onClick={() => { setReportMonth((c) => c.m === 11 ? { y: c.y + 1, m: 0 } : { y: c.y, m: c.m + 1 }); setSelectedReportDay(null); }}
                  style={{ border: `1px solid ${p.pillBorder}`, borderRadius: 20, padding: '5px 8px', color: p.tmid, background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 3, marginBottom: 5 }}>
                {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                  <div key={i} style={{ textAlign: 'center', fontSize: 8, color: p.tmid, fontWeight: 600, padding: '3px 0' }}>{d}</div>
                ))}
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 3 }}>
                {reportCells.map((day, i) => {
                  if (day === null) return <div key={`b${i}`} />;
                  const dateStr = isoDate(reportMonth.y, reportMonth.m, day);
                  const rec = meetingDaysByDate.get(dateStr);
                  const isToday = dateStr === reportToday;
                  const isSelected = dateStr === selectedReportDay;
                  const key = rec ? timelineKeyForStatus(rec.myStatus) : null;
                  const isNA = rec?.myStatus === 'not_required';
                  return (
                    <button
                      key={dateStr}
                      type="button"
                      onClick={() => setSelectedReportDay(rec ? dateStr : null)}
                      style={{
                        aspectRatio: '1',
                        borderRadius: 8,
                        border: `1px solid ${isSelected ? p.green : isToday ? p.tdH : 'transparent'}`,
                        background: isSelected ? p.greenDeep : 'transparent',
                    ...
                    >
                      <span style={{ fontSize: 9, color: rec ? p.tl : p.tsub, fontWeight: isToday ? 700 : 500 }}>{day}</span>
                      {key && <span style={{ width: 4, height: 4, borderRadius: '50%', background: meetingDotColor(p, key) }} />}
                      {isNA && <span style={{ width: 4, height: 4, borderRadius: '50%', background: p.tmid }} />}
                      {rec && <span style={{ fontSize: 6, color: p.tsub, lineHeight: 1 }}>{rec.presentCount}</span>}
                    </button>
                  );
                })}
              </div>

              <div style={{ display: 'flex', gap: 10, marginTop: 12, paddingTop: 12, borderTop: `1px solid ${p.border}`, flexWrap: 'wrap' }}>
                {(['present', 'late', 'excused', 'absent'] as TimelineKey[]).map((k) => (
                  <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: meetingDotColor(p, k) }} />
                    <span style={{ fontSize: 8.5, color: p.tsub }}>{k.charAt(0).toUpperCase() + k.slice(1)}</span>
                  </div>
                ))}
              </div>
              ...
              {selectedReportRow ? (
                <div style={{ marginTop: 12, background: p.lightCard, borderRadius: 12, padding: 10 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 9.5, fontWeight: 700, color: p.td, background: p.dark, padding: '3px 9px', borderRadius: 20 }}>
                ...
                <div style={{ fontSize: 10.5, color: p.tsub, marginTop: 12, textAlign: 'center' }}>No past meetings recorded yet.</div>
              ) : (
                <div style={{ fontSize: 9.5, color: p.tsub, marginTop: 12, textAlign: 'center' }}>Tap a marked day for details.</div>
              )}
            </div>
