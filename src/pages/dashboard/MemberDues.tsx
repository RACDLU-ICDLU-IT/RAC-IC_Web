...
// Explicit tenant-id -> club-type lookup, matching the two configs
// registered in tenants/index.ts (racdlu.config / icdlu.config). Kept
// local to this component since TenantConfig itself has no club-type
// field to read from. Unlike a boolean check (`tenant.id === 'racdlu'
// ? rotaract : interact`), an unrecognized tenant id here falls through
// to an explicit default WITH a console warning, instead of silently
// mis-theming a future third tenant as 'interact'.
const CLUB_BY_TENANT: Record<string, keyof typeof PALETTE> = {
  racdlu: 'rotaract',
  icdlu: 'interact',
};
...
function resolveClub(tenantId: string): keyof typeof PALETTE {
  ...
  if (!club) {
    console.warn(
      `[MemberDues] Unrecognized tenant id "${tenantId}" — no palette mapping in CLUB_BY_TENANT. ` +
      `Falling back to "${DEFAULT_CLUB}". Add this tenant to CLUB_BY_TENANT to fix its branding.`
    );
    return DEFAULT_CLUB;
  }
  return club;
...
export default function MemberDues() {
  ...
  const { fetchMemberLedger, submitPayment, fetchDefaultBkashNumber, loading } = useDues();
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        setSubmitError("Couldn't submit — check the details and try again.");
      }
    } finally {
      setSubmitting(false);
    }
      ...
      {payEntry && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: p.av2, marginBottom: 16 }}>{formatAmount(payEntry.amount, payEntry.currency)}</div>
          ...
          >
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ marginTop: 14, padding: 14, borderRadius: 14, background: p.lightCard, color: p.td, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 18, fontWeight: 700 }}>{formatAmount(receiptEntry.amount, receiptEntry.currency)}</span>
              </div>
              <button
                type="button"
                onClick={() => setReceiptEntry(null)}
