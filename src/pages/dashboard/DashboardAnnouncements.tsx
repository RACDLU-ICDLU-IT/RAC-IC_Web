 ...
 *
 * --------------------------- DEEP LINKING ---------------------------
 * Supports being opened as /dashboard/announcements?id=<announcement_id>
 * (used by DashboardHome.tsx's "Needs attention" card, which links
 * straight to a specific announcement rather than just this page's
 * default list view). On load, once `sorted` is available, if the id
 * in the URL matches a visible announcement it's auto-expanded,
 * marked read, and scrolled into view — see the deep-link effect
 * below. If the id doesn't match anything visible (wrong tenant,
 * expired since the link was shown, audience no longer matches, or a
 * stale/invalid id), this fails silently into the normal list view
 * rather than showing an error — the id param is a convenience, not a
 * hard requirement to render the page.
 * ------------------------------------------------------------------
 */
...
export default function DashboardAnnouncements() {
  ...
  // Deep-link support: /dashboard/announcements?id=<announcement_id>
  // (see header comment). searchParams is only read from here — this
  // page never writes to the URL itself except to clear the param
  // once the deep link has been consumed (see the effect below), so a
  // page refresh doesn't keep re-scrolling/re-expanding on every load.
  ...
  /**
   * Deep-link consumption: once `sorted` reflects the fetched +
   * filtered data (i.e. loading has finished at least once), look for
   * `deepLinkId` among visible announcements. If found: expand it,
   * mark it read, and scroll its card into view. Runs at most once
   * per page load (consumedDeepLinkRef) — this is deliberately NOT
   * re-run every time `sorted` changes (e.g. after markAsRead updates
   * readIds and the array is recomputed), since re-triggering the
   * scroll/expand on every subsequent re-render would fight the user
   * if they've since collapsed the card or scrolled away themselves.
   *
   * If `deepLinkId` doesn't match anything in `sorted` (wrong tenant,
   * expired since the link was generated, audience no longer
   * matches, or just a bad id), this intentionally does nothing
   * further — no error banner, since a missing/stale id is not a
   * failure of this page, just an id that doesn't resolve to
   * anything visible right now.
   */
  useEffect(() => {
    if (!deepLinkId || loading || consumedDeepLinkRef.current) return;
    const match = sorted.find(a => a.id === deepLinkId);
    if (!match) {
      // Nothing (yet) to consume — leave the ref unset in case `sorted`
      // is still settling (e.g. announcements arrived before the
      // reads query resolved); this effect re-runs on the next
      // `sorted`/`loading` change and will try again.
      return;
    }
    ...
    // Scroll the matched card into view once its expanded content has
    // had a frame to render (so the scroll target's height is already
    // correct, not mid-expand-animation).
    requestAnimationFrame(() => {
      cardRefs.current[match.id]?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    });

    // Clear the `id` param from the URL now that the deep link has
    // been consumed — keeps a page refresh from re-triggering the
    // same scroll/expand every time, and keeps the URL clean if the
    // member navigates elsewhere and back via browser history.
    setSearchParams(
      prev => {
  ...
  }, [deepLinkId, loading, sorted, markAsRead, setSearchParams]);
